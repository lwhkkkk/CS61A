...
def berry_finder(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    if label(t) == 'berry': #== 用来比较值，is用来比较内存
        return True
    ...
    1
      2
        3
          ...
          2
    """
    "*** YOUR CODE HERE ***"#还是要递归才能走到最底下 ，注意tree的用法 
    if is_leaf(t) == True:
        return tree(label(t),[tree(x) for x in leaves ])
    else:
        new_branchs = []
        for branch in branches(t):
            branchs =sprout_leaves(branch,leaves)
            new_branchs.append(branchs)
