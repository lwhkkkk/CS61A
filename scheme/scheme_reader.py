...
def scheme_read(src):
    ...
    if src.current() is None:
        raise EOFError
    val = src.pop_first() # Get and remove the first token，直接移除第一个元素
    if val == 'nil':
        # BEGIN PROBLEM 1
        "*** YOUR CODE HERE ***"
        return nil
        ...
        # BEGIN PROBLEM 1
        "*** YOUR CODE HERE ***"
        return read_tail(src) #递归调用，要return
        # END PROBLEM 1
    elif val == "'":
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        rest_of_expr = scheme_read(src) #递归调用 后面的内容
        return Pair('quote', Pair(rest_of_expr,nil))
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            rest_of_list = read_tail(src)
            return Pair(first_element,rest_of_list)
