...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    """
    # Evaluate atoms
    if scheme_symbolp(expr): #如果是符号x 去环境查值
        return env.lookup(expr)
    elif self_evaluating(expr): #如果是自己的值（4），直接return
        return expr
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        #如果是整个操作式，就递归调用scheme_eval
        proc = scheme_eval(first, env) #递归调用自己的，查找first第一个操作符的实例
        validate_procedure(proc)   
        if  isinstance(proc,MacroProcedure):
            new_expr = proc.apply_macro(rest,env)
            return scheme_eval(new_expr,env,True)
        else:
            computed_operands = rest.map(lambda x :scheme_eval(x,env))#剩下的每个参数都放进去schem_eavl进行计算
            return scheme_apply(proc,computed_operands,env) #这里的porc是创建的lambda实例或者define实例
...
def eval_all(expressions, env): 
    ...
    """
    # BEGIN PROBLEM 7 
    if expressions is nil: #如果本身为nil，expressions前面的first直接没有，会报错
        return None
    ptr = expressions 
    while ptr is not nil:
        if ptr.rest is nil:
            return scheme_eval(ptr.first,env,True) #如果是最后一个，计算并返回
        else: 
            scheme_eval(ptr.first,env,False) #求值但是不return
            ptr = ptr.rest
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        if symbol in  self.bindings:
            return self.bindings[symbol] #注意字典的用法

        elif self.parent is not None:
            return self.parent.lookup(symbol) #递归去父类查找symbol
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        child = Frame(self) #通过这个创建一个子frame
        while formals is not nil: 
            child.define(formals.first,vals.first)
            formals = formals.rest #往后移动
            vals= vals.rest
        return child
        # END PROBLEM 10
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        # BEGIN PROBLEM 3
        "*** YOUR CODE HERE ***"
        while args is not nil: #注意args的结尾为nil
            python_args.append(args.first)
            args = args.rest 
          
        if self.use_env == True:
            python_args.append(env) #如果为True，说明有环境，把env存进去
...
class LambdaProcedure(Procedure):
    ...
    def __init__(self, formals, body, env):
        ...
        self.formals = formals #形式参数（函数希望接受的参数名字列表）
        self.body = body #函数体，需按顺序执行后面所有的后续表达式列表
        self.env = env
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        return self.env.make_child_frame(self.formals,args) #self.env是lambda函数定义是的环境，env是调用时候的环境
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        fname = target.first #函数名
        flist = target.rest #参数列表
        newlist = Pair(flist, expressions.rest)
        lname = do_lambda_form(newlist,env)
        env.define(fname,lname)
        return fname
        # END PROBLEM 9
    else:
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    return  expressions.first #把quote 的pair里面的前面拿走，nil丢掉
    # END PROBLEM 6
...
def do_if_form(expressions, env):
    ...
    if is_true_primitive(scheme_eval(expressions.first, env)): #判断结果为true
        return scheme_eval(expressions.rest.first, env,True)
    elif len(expressions) == 3: #判断结果为false
...
def do_and_form(expressions, env):
    ...
    ptr = expressions
    while ptr is not nil:
        if ptr.rest is nil: #最后一个表达式，处理尾部递归求值并返回
            return scheme_eval(ptr.first,env,True)

        
        current_val = scheme_eval(ptr.first,env,False)
        if is_false_primitive(current_val):
            return current_val
        #指针往后移动
        ptr= ptr.rest
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    while ptr is not nil:
        ...
        current_val = scheme_eval(ptr.first,env)
        if is_true_primitive(current_val):
            return current_val
...
def make_let_frame(bindings, env):
    ...
    name = []
    value = []
    while bindings is not nil: #每个bindings是一个pair链表
        binding = bindings.first #取第一个链表
        validate_form(binding,2,2)
        name.append(binding.first)
        value.append(scheme_eval(binding.rest.first,env)) #运算后面那块
        bindings = bindings.rest
    for n in reversed(name): 
        names = Pair(n,names) #旧的链表绑在后面
...
class MuProcedure(Procedure):
    ...
    #动态定义域创建env
    def make_call_frame(self, args, env):
        return env.make_child_frame(self.formals,args) #这里用传进来的env来创建子帧
...
def do_mu_form(expressions, env):
    ...
    body = expressions.rest
    if body is not nil:
        return MuProcedure(formals,body) #传的参数主要看_init_
    # END PROBLEM 18
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        """
        if tail and not scheme_symbolp(expr) and not self_evaluating(expr): #为True进入
        ...
        while isinstance(result,Thunk): #检查result是不是Thunk，这里用while就不会递归爆栈
