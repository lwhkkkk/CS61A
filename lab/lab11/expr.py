...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        if self.var_name in env : #查到是否在字典里面
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        proc = self.operator.eval(env)
        val_operands = [ x.eval(env) for x in self.operands] #operands是列表，所以用列表推导式一个取出来再放进列表
        #这里还存在一个递归，如果是CallExpr的.eval会进入递归
        return proc.apply(val_operands)
...
class LambdaFunction(Value):
    ...
    def __init__(self, parameters, body, parent):
        ...
        self.parameters = parameters #这是定义函数是的参数
        self.body = body #函数体是外面传进来的，是Expr类，有eval方法
        self.parent = parent
    #这道题目不是很理解
    def apply(self, arguments): #这里的arguments是调用定义函数的时的参数
        """
        >>> from reader import read
        ...
        #通过zip将形参和实参对其并塞入局部环境字典中
        for p,a in zip(self.parameters,arguments): #sself.parameters是字符串参数名，arguments是具体数值对象
            local_env[p] = a
        #让函数体在这个局部环境中求值
