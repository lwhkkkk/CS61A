...
class Card:
    ...
    def power(self, other_card):
        ...
        score = self_attack - other_defense /2 
        return score
...
class Player:
    def __init__(self, deck, name):
        ...
        pai = []
        for i in range(5):
            card = self.deck.draw() #更加明确在操作玩家自己绑定的牌库
            pai.append(card)
        self.hand = pai
    ...
    def play(self, card_index):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.hand.pop(card_index) #remove是移除，pop是找到index并把里面的值return出去
...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        for _ in range(3):
            opponent.hand.pop(0) #注意pop只能一次推出去一个，所以要做for循环
            opponent.draw() #这里不要重复造轮子，把之前写好的draw直接拿来用
...
class TACard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        change = other_card.defense
        other_card.defense = other_card.attack
        other_card.attack = change
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        for card in player.deck.cards: #注意这里直接用列表循环，不用长度了，然后用card去除列表中的元素
            card.attack = other_card.attack + card.attack
            card.defense = other_card.defense + card.defense

        for card in opponent.deck.cards[:]: #注意这里用[:]创建一个副本，避免for循环跳过了remove后位置往前移动的元素
            if card.attack == other_card.attack or card.defense == other_card.defense :
                opponent.deck.cards.remove(card) #这里直接remove元素
